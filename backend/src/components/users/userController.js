...
const deleteUserByIdAndType = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ message: `User with ID ${_id} deleted successfully` });
    } else {
...
const addGovernorOrAdmin = async (req, res) => {
  ...
  if (!username || !password || !type) {
    return res
      .status(400)
      .json({ message: "Username and password and usertype are required." });
  }

  try {
    // Send data to the service layer to create a new tourismGovernor
    const result = await userService.addGovernorOrAdmin(
      username,
      password,
      type
    );
...
const createTourGuide = async (req, res) => {
  try {
    const tourGuide = await userService.createTourGuide(
      req.params._id,
      req.body
    );
    if (!tourGuide) {
...
const updateTourGuide = async (req, res) => {
  try {
    const updatedTourGuide = await userService.updateTourGuide(
      req.params._id,
      req.body
    );
    if (!updatedTourGuide) {
...
const createAdvertiser = async (req, res) => {
  ...
  try {
    ...
    const advertiser = await userService.createAdvertiser(
      req.params._id,
      req.body
    );
    if (!advertiser) {
...
const updateAdvertiser = async (req, res) => {
  try {
    const updatedAdvertiser = await userService.updateAdvertiser(
      req.params._id,
      req.body
    );
...
const updateSeller = async (req, res) => {
  try {
    const updatedSeller = await userService.updateSeller(
      req.params._id,
      req.body
    );
    if (!updatedSeller) {
...
const updateTourist = async (req, res) => {
  try {
    ...
    const updatedTourist = await userService.updateTourist(
      req.params._id,
      updateData
    );
    if (!updatedTourist) {
...
const rateTourGuide = async (req, res) => {
  ...
  try {
    const result = await userService.rateTourGuide(
      touristId,
      tourGuideId,
      itineraryId,
      rating
...
const commentOnTourGuide = async (req, res) => {
  ...
  try {
    const result = await userService.commentOnTourGuide(
      touristId,
      tourGuideId,
      itineraryId,
      commentText
...
const rateItinerary = async (req, res) => {
  ...
  try {
    const result = await userService.rateItinerary(
      touristId,
      tourGuideId,
      itineraryId,
      rating
...
const commentOnItinerary = async (req, res) => {
  ...
  try {
    const result = await userService.commentOnItinerary(
      touristId,
      tourGuideId,
      itineraryId,
      commentText
...
const rateActivity = async (req, res) => {
  ...
  try {
    const result = await userService.rateActivity(
      touristId,
      activityId,
      rating
    );
...
const commentOnActivity = async (req, res) => {
  ...
  try {
    const result = await userService.commentOnActivity(
      touristId,
      activityId,
      commentText
    );
...
const rateHistoricalPlace = async (req, res) => {
  ...
  try {
    const result = await userService.rateHistoricalPlace(
      touristId,
      historicalPlaceId,
      rating
    );
...
const commentOnHistoricalPlace = async (req, res) => {
  ...
  try {
    const result = await userService.commentOnHistoricalPlace(
      touristId,
      historicalPlaceId,
      commentText
    );
...
const redeemPoints = async (req, res) => {
  ...
  if (!userId || !points) {
    return res
      .status(400)
      .json({ message: "Missing userId or points parameter." });
  }
  try {
...
const pointsAfterPayment = async (req, res) => {
  ...
  if (!userId || !amount) {
    return res
      .status(400)
      .json({ message: "Missing userId or amount parameter." });
  }
  try {
...
const getLevel = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Level fetched successfully", level });
  } catch (error) {
...
const acceptTerms = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Terms and conditions accepted.", user: result });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
const requestDeletion = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Request to be deleted accepted.", user: result });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
...
const registerUser = async (req, res) => {
  ...
  const { email, username, password, mobileNum, nation, dob, profession } =
    req.body;
  if (!type) {
  ...
  if (type == "tourist") {
    ...
    try {
      const result = await userService.registerTourist(
        email,
        username,
        password,
        mobileNum,
...
const updatePromoCode = async (req, res) => {
  try {
    ...
    const birthdayTourists = await Promise.all(
    ...
    if (birthdayTouristIds.length === 0) {
      return res.status(404).json({
        message:
          "No tourists with birthdays today or all have already received a promo code",
      });
    }

    // Pass the tourist IDs to update the promo code
    const promoCodeResult = await userService.updatePromoCode(
      birthdayTouristIds
    );
...
const changePasswordAfterOTP = async (req, res) => {
  ...
  try {
    const result = await userService.changePasswordAfterOTP(
      userId,
      newPassword
    );
    return res.status(200).json(result);
