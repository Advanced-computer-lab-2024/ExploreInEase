...
const updateTourist = async (id, updateData) => {
  return await Tourist.findByIdAndUpdate(id, updateData, {
    new: true,
    runValidators: true,
  });
};
...
const hasCompletedItinerary = async (touristId, ItineraryId, guideId) => {
  try {
    ...
    if (!tourist) {
      ...
      throw new Error(
        "Itinerary is not found or does not belong to the specified tour guide"
      );
    } else {
      // Check if any of the itinerary dates have passed
      const now = new Date();
      const hasDatePassed = itinerary.dateTimeAvailable.some(
        (date) => date < now
      );
      if (!hasDatePassed) {
...
const hasAttendedActivity = async (touristId, ActivityId) => {
  try {
    ...
    if (!tourist) {
      throw new Error(
        "No such activity exists or the tourist did not sign up for this activity"
      );
    } else {
...
const updateHistoricalPlacesRatings = async (
  historicalPlaceId,
  updatedFields
) => {
  try {
...
const updateHistoricalPlacesComments = async (
  historicalPlaceId,
  updateOperation
) => {
  try {
...
const checkTouristDeletionCriteria = async (id) => {
  ...
  for (const itineraryObj of tourist.itineraryId) {
    ...
    if (
      itinerary &&
      itinerary.dateTimeAvailable &&
      itinerary.dateTimeAvailable.some((date) => new Date(date) > now)
    ) {
      return false;
    }
  }
...
const getTouristReport = async (user) => {
  try {
    ...
    const totalTouristinEventObject = eventObject.reduce(
      (total, event) => total + event.totalPeople,
      0
    );
