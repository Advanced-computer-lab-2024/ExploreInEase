...
const rateTourGuide = async (touristId, tourGuideId, itineraryId, rating) => {
  try {
    ...
    const hasCompleted = await userRepository.hasCompletedItinerary(
      touristId,
      itineraryId,
      tourGuideId
    );
    ...
    if (!hasCompleted) {
      throw new Error(
        "You cannot rate this tour guide because you havent attended an itinerary with them yet."
      );
    }

    // Retrieve the tour guide directly by querying the user repository
    const tourGuide = await Users.findOne({
      _id: tourGuideId,
      type: "tourGuide",
    });
    if (!tourGuide) {
...
const commentOnTourGuide = async (
  touristId,
  tourGuideId,
  itineraryId,
  commentText
  ...
  try {
    ...
    if (!hasCompleted) {
      throw new Error(
        "You cannot comment on this tour guide because you haven't completed this itinerary with them."
      );
    }
    ...
    await userRepository.updateTourGuideComments(tourGuideId, {
      comment: tourGuide.comment,
    });
...
const commentOnItinerary = async (
  touristId,
  tourGuideId,
  itineraryId,
  commentText
  ...
  try {
    ...
    if (!hasCompleted) {
      throw new Error(
        "You cannot comment on this itinerary because you haven't completed this itinerary yet."
      );
    }
    ...
    const updatedItinerary = await userRepository.updateItineraryComments(
      itineraryId,
...
const rateItinerary = async (touristId, tourGuideId, itineraryId, rating) => {
  try {
    ...
    if (!hasCompleted) {
      throw new Error(
        "You cannot rate this itinerary because you havent attended it yet."
      );
    }
...
const rateActivity = async (touristId, activityId, rating) => {
  try {
    // Check if the tourist has completed the itinerary with the specified tour guide
    const hasAttended = await userRepository.hasAttendedActivity(
      touristId,
      activityId
    );
    ...
    if (!hasAttended) {
      throw new Error(
        "You cannot rate this activity because you havent attended this activity yet."
      );
    }
...
const commentOnActivity = async (touristId, activityId, commentText) => {
  try {
    ...
    if (!hasAttended) {
      throw new Error(
        "You cannot comment on this Activity because you haven't attended this activity yet."
      );
    }
    ...
    // Use the repository method to push the new comment to the comments array
    const updatedActivity = await userRepository.updateActivityComments(
      activityId,
      { $push: { comments: newComment } }
    );
...
const rateHistoricalPlace = async (touristId, historicalPlaceId, rating) => {
  try {
    ...
    const historicalPlace = await HistoricalPlace.findOne({
      _id: historicalPlaceId,
    });
    if (!historicalPlace) {
    ...
    historicalPlace.rating =
      historicalPlace.ratingSum / historicalPlace.ratingCount; // Update the average rating
    ...
    return {
      message: "Rating added successfully",
      updatedHistoricalPlace: historicalPlace,
    };
  } catch (error) {
...
const commentOnHistoricalPlace = async (
  touristId,
  historicalPlaceId,
  commentText
) => {
  try {
    ...
    const updatedHistoricalPlace =
      await userRepository.updateHistoricalPlacesComments(historicalPlaceId, {
        $push: { comments: newComment },
      });
...
const updatingStatusUser = async (userId, status) => {
  ...
  return {
    status: 200,
    response: { message: "Status updated successfully", user: newUser },
  };
};
...
const changePassword = async (userId, oldPassword, newPassword) => {
  ...
  if (user) {
    ...
    return {
      status: 200,
      response: { message: "Password updated successfully", user: newUser },
    };
  } else {
    if (tourist) {
      ...
      return {
        status: 200,
        response: { message: "Password updated successfully", user: tourist },
      };
    } else {
...
const redeemPoints = async (userId, points) => {
  ...
  return {
    ...
    response: {
      ...
      leftPoints: userNewPoints,
      redeemedPoints: amount,
      user: userAfterRedeemed,
    },
  };
};
...
const pointsAfterPayment = async (userId, amount) => {
  ...
  const userAfterPoints = await userRepository.pointsAfterPayment(
    userId,
    addedPoints
  );
...
const registerTourist = async (
  ...
  return {
    ...
    response: {
      message: "Turist registered successfully",
...
const registerUser = async (type, email, username, password) => {
  try {
    ...
    return {
      status: 200,
      response: { message: "User registered successfully", User: savedUser },
    };
  } catch (error) {
...
const updatePromoCode = async (birthdayTourists) => {
  ...
  const promoCodeResult = await userRepository.updatePromoCode(
    birthdayTourists,
    assignedPromoCodes
  );
  return promoCodeResult;
...
const addInterestedIn = async (userId, eventId, eventType) => {
  ...
  const updatedUser = await userRepository.addInterestedIn(
    user,
    eventId,
    eventType
  );
